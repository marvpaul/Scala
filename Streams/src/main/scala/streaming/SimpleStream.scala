 ...
 *******************************************************************************************/
 ...
 def startsWith[A](s:SimpleStream[A]): Boolean= this match{
  		  
  	case Cons(_,_) if (!s.isEmpty) => if (s.head==head) tail().startsWith(s.tail()) else false
  	case Cons(_,_) if (s.isEmpty) => true
  	case _ => false
 }
}
 ...
 *******************************************************************************************/
 ...
 def createStreamRange(start:Int, end:Int): SimpleStream[Int]= 
   if (start>end) Empty
     else SimpleStream.cons(start, createStreamRange(start+1, end))
